...
export default function ApartmentCardsContainer({isFlipped, handleFLip, onUpdate}) {
  ...
  return (
    <div className='flex gap-5 flex-wrap w-full '>
        {loading ? <Skeleton width={300} height={150}/>: (payload && payload.apartments.map((apart =><ApartmentCard key={apart._id} apartment={apart} isFlipped={isFlipped[apart._id]} handleFLip={()=>handleFLip(apart._id)} onUpdate={onUpdate}/>)))}
    </div>
  )
}
