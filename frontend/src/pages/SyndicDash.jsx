...
export default function SyndicDash() {
  ...
  const fac = useSelector(state=>state.getBills.payload)
        ...
        <div className='py-2 px-10 w-full overflow-hidden relative'>
          <div className='bg-[#d9d9d99a] w-full h-[600px] rounded-2xl flex flex-col gap-5 p-5'>
            <div className='w-full flex justify-between'>
              <Card bg="bg-[#5E35B1]" circleB="bg-[#4A2597]" circleS="bg-[#532AA5]" income={`$${loading ? 0 : payload && payload.apartmentsWithBills.length * 50}`} label="Monthly Earnings" icon="mdi:cash-register" />
              <Card bg="bg-[#1E88E5]" circleB="bg-[#096CC2]" circleS="bg-[#7DBBF1]" income={loading ? 0 : payload && payload.apartmentsWithBills.length} label="Reserved Apartments" icon="ant-design:home-filled" />
              <Card bg="bg-[#FFCB42]" circleB="bg-[#FFB800]" circleS="bg-[#FFEBB7]" income={loading ? 0 : fac && fac.bills.length} label="Factures" icon="uil:bill" />
            </div>
            <div className='h-96 overflow-y-auto'>
              <div className='min-h-full w-full bg-white overflow-y-auto rounded-xl p-5 relative'>
                <ApartmentCardsContainer onUpdate={handleUpdate} isFlipped={isFlipped} handleFLip={handleFLip} fac={fac}/>
